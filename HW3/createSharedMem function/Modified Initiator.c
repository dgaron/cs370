...
int createSharedMemory(const char *shmName, char *sharedMemPtr) {
    // Create shared memory segment
    int shm_fd = shm_open(shmNames[i], O_CREAT | O_RDWR, 0666);
       
    ftruncate(shm_fd, SHM_SIZE);
      
    sharedMemPtr = mmap(0, SHM_SIZE, PROT_READ, MAP_SHARED, shm_fd, 0);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        shm_fd = createSharedMemory(shmNames[i], ptr[i]);
